...
public class Phase2Loop {
    ...
    public Phase2Loop(MainCharacter player, Set<KeyCode> activeKeys, List<ImageView> obstacles, AnchorPane phase2World, List<Monster> monster, ImageView monsterBoomImage, Phase2Controller controller, ImageView gate) {
        this.player = player;
        this.phase2World = phase2World;
        // Add mouse event handler to the scene
        phase2World.setOnMousePressed(this::handleMousePress);
        this.WORLD_WIDTH = phase2World.getPrefWidth();
        this.WORLD_HEIGHT = phase2World.getPrefHeight();
        ...
        gameLoop = new AnimationTimer() {
            ...
            public void handle(long now) {
                if (now - lastUpdate >= 150_000_000) {
                    ...
                    // Check if monster 27 is dead
                    boolean isMonster27Dead = monsters.stream()
                            .filter(monster -> monster.monsterId == 27)
                            .noneMatch(Monster::isAlive);

                    if (isMonster27Dead && !controller.isPetGiftCollected) {
                        // Make mysteryPet AnchorPane visible
                        controller.mysteryPet.setVisible(true);
                    }
                                ...
                                if (monster.monsterId == 13 || monster.monsterId == 14 || monster.monsterId == 15 || monster.monsterId == 16 || monster.monsterId == 17) {
                                    ...
                                    if (monster.isPlayerInAttackZone(player)) {
                                        if (now - lastAttackTime >= attackCooldown) {
                                            ...
                                            if (monster.monsterId != 18 || monster.monsterId != 19|| monster.monsterId != 22|| monster.monsterId != 26|| monster.monsterId != 28) {
                                                player.setHealth(newPlayerHealth);
                                                lastAttackTime = now;
                                            }
                                        }
    ...
    private void checkPosition() {
        ...
        if ((x >= 1710 && x <= 1917 && y >= 580 && y <= 880)) {
            controller.checkLevel3Entry();
        }
    ...
    private void checkMonstersAndGate() {
        synchronized (monsters) {
            ...
            if (allRelevantMonstersDead) {
                ...
                if (currentGate != null) {
                    phase2World.getChildren().remove(currentGate);
    ...
    private void moveWorld(int deltaX, int deltaY) {
        ...
        if (newWorldOffsetX <= (WORLD_WIDTH - SCREEN_WIDTH) - 144 && newWorldOffsetX >= SCREEN_WIDTH - WORLD_WIDTH + (WORLD_WIDTH - SCREEN_WIDTH) - 144) {
            if (deltaX < 0 && player.getPosX() <= WORLD_WIDTH - ((SCREEN_WIDTH / 2) + (48 / 2.0))) {
                ...
                phase2World.setTranslateX(worldOffsetX);
            }
        ...
        if (newWorldOffsetY <= (WORLD_HEIGHT - SCREEN_HEIGHT) - 144 && newWorldOffsetY >= SCREEN_HEIGHT - WORLD_HEIGHT + (WORLD_HEIGHT - SCREEN_HEIGHT) - 144) {
            if (deltaY < 0 && player.getPosY() <= WORLD_HEIGHT - (SCREEN_HEIGHT / 2) - (48 / 2.0)) {
                ...
                phase2World.setTranslateY(worldOffsetY);
            }
...
// projectileImage.setVisible(false);
                phase2World.getChildren().add(projectileImage); // Add to the scene
...
// No need to adjust for LEFT and RIGHT
    ...
    private void respawnMonster21() {
        ...
        ImageView monster21 = (ImageView) phase2World.lookup("#monster21");
        ImageView monsterHealthBar21 = (ImageView) phase2World.lookup("#monsterHealthBar21");

        if (monster21 != null && monsterHealthBar21 != null) {
            Monster monster = new Monster(monster21, 0, 1874, 1248, 10, 100, 300, 1874, 1248, 1874, 1248, 21, monsterHealthBar21);
            monster.setGameWorld(phase2World);
    ...
    private void respawnMonster20() {
        ...
        ImageView monster20 = (ImageView) phase2World.lookup("#monster20");

        // Get the monsterHealthBar8 ImageView from the FXML
        ImageView monsterHealthBar20 = (ImageView) phase2World.lookup("#monsterHealthBar20");

        // Make sure the ImageView was found
        if (monster20 != null && monsterHealthBar20 != null) {
            Monster monster = new Monster(monster20, 0, 2842, 1740, 10, 100, 300, 2842, 1740, 2842, 1740, 20, monsterHealthBar20);
            monster.setGameWorld(phase2World);
            monsters.add(monster);
    ...
    private void launchMonsterProjectile(Monster monster, MainCharacter player) {
        if (monster.monsterId == 19 && (monster.getHealth() <= monster.getInitialHealth() / 2))  { // Only for monster with ID 19
            ...
            for (int i = 0; i < numProjectiles; i++) {
                ...
                phase2World.getChildren().add(projectileImage);
