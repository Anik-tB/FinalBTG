...
public class Level1Loop {
    ...
    public Level1Loop(MainCharacter player, Set<KeyCode> activeKeys, List<ImageView> obstacles, AnchorPane level1World, List<loginMonster> monster, ImageView monsterBoomImage, Level1Controller controller, ImageView gate) {
        this.player = player;
        this.level1World = level1World;
        // Add mouse event handler to the scene
        level1World.setOnMousePressed(this::handleMousePress);
        this.WORLD_WIDTH = level1World.getPrefWidth();
        this.WORLD_HEIGHT = level1World.getPrefHeight();
    ...
    private void checkMonstersAndGate() {
        synchronized (monsters) {
            ...
            if (allRelevantMonstersDead) {
                ...
                if (currentGate != null) {
                    level1World.getChildren().remove(currentGate);
    ...
    private void moveWorld(int deltaX, int deltaY) {
        ...
        if (newWorldOffsetX <= (WORLD_WIDTH - SCREEN_WIDTH) - 144 && newWorldOffsetX >= SCREEN_WIDTH - WORLD_WIDTH + (WORLD_WIDTH - SCREEN_WIDTH) - 144) {
            if (deltaX < 0 && player.getPosX() <= WORLD_WIDTH - ((SCREEN_WIDTH / 2) + (48 / 2.0))) {
                ...
                level1World.setTranslateX(worldOffsetX);
            }
        ...
        if (newWorldOffsetY <= (WORLD_HEIGHT - SCREEN_HEIGHT) - 144 && newWorldOffsetY >= SCREEN_HEIGHT - WORLD_HEIGHT + (WORLD_HEIGHT - SCREEN_HEIGHT) - 144) {
            if (deltaY < 0 && player.getPosY() <= WORLD_HEIGHT - (SCREEN_HEIGHT / 2) - (48 / 2.0)) {
                ...
                level1World.setTranslateY(worldOffsetY);
            }
    ...
    private void respawnMonster9() {
        ...
        ImageView monster12 = (ImageView) level1World.lookup("#monster9");
        ImageView monsterHealthBar9 = (ImageView) level1World.lookup("#monsterHealthBar9");

        if (monster12 != null && monsterHealthBar9 != null) {
            loginMonster monster = new loginMonster(monster12, 0, 1874, 1248, 10, 100, 300, 1874, 1248, 1874, 1248, 9, monsterHealthBar9);
            monster.setGameWorld(level1World);
    ...
    private void respawnMonster() {
        ...
        ImageView monster11 = (ImageView)level1World.lookup("#monster8");

        // Get the monsterHealthBar8 ImageView from the FXML
        ImageView monsterHealthBar8 = (ImageView) level1World.lookup("#monsterHealthBar8");

        // Make sure the ImageView was found
        if (monster11 != null && monsterHealthBar8 != null) {
            loginMonster monster = new loginMonster(monster11, 0, 1654, 828, 10, 100, 300, 1654, 828, 1654, 828, 8, monsterHealthBar8);
            monster.setGameWorld(level1World);
            monsters.add(monster);
    ...
    private void launchMonsterProjectile(loginMonster monster, MainCharacter player) {
        ...
        level1World.getChildren().add(projectileImage);
